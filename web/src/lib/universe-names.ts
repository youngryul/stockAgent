...
const NAME_BY_SYMBOL: Record<string, string> = Object.fromEntries(
  UNIVERSE.map((item) => [item.symbol, item.name]),
);

const MARKET_BY_SYMBOL: Record<string, "KR" | "US"> = Object.fromEntries(
 ...
 */
export function lookupUniverseName(symbol: string): string {
  return NAME_BY_SYMBOL[symbol] || "";
}

/**
 * Look up KR/US for a known ticker.
 * @param symbol - yfinance ticker
 */
